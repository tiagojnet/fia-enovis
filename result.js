...
function copyValues() {
    ...
    text += langData['measured'] + " " + document.getElementById("outputFeno").innerHTML + "\n";
    text += langData['height'] + " " + document.getElementById("outputHeight").innerHTML + "\n";
    text += langData['age'] + " " + document.getElementById("outputAge").innerHTML + "\n";
    text += langData['sex'] + " " + document.getElementById("outputSex").innerHTML + "\n";
    text += langData['predicted'] + " " + document.getElementById("predictedFeno").innerHTML + "\n";
    text += langData['percent predicted'] + " " + document.getElementById("percFeno").innerHTML + "\n";
...
function setFenoValues() {
    ...
    if (!feno || !height || !globalAge || !patientsex || feno < 5 || feno > 300 || isNaN(feno)
        || height < 80 || height > 250 || isNaN(height) || globalAge < 5 || globalAge > 80  || isNaN(globalAge) 
    ...
    document.getElementById("predictedFeno").innerHTML = Math.round(predFeno) + ' ppb';
    ...
    if (widthPercentage > 100) {
        ...
        document.getElementById("floatingPerc").innerHTML = Math.round(percFeno) + '%';
    }
    
    document.getElementById("line").style.width = widthPercentage + '%';
    document.getElementById("floatingPerc").style.paddingLeft = (widthPercentage - 5) + '%';

    const valueAdj = percFeno > 122 ? langData["elevated"] : langData["normal"]

    document.getElementById("main-msg").innerHTML = langData["mainMsg1"] + feno + " ppb (" +  Math.round(percFeno) + "%) " + langData["mainMsg2"] + " " + valueAdj;
...
function translateItems() {
    ...
    if (document.getElementById("main-msg")){
        ...
        document.getElementById("main-msg").innerHTML = 
            langData["mainMsg1"] + feno + " ppb (" +  Math.round(percFeno) + "%) " + langData["mainMsg2"] + " " + valueAdj + langData["mainMsg3"];
    }
    ...
    if (document.getElementById("outputSex")) {
        document.getElementById("outputSex").innerHTML = langData['patientsex'][patientsex];
    }
...
function printPage() {
    document.getElementById("outputDate").innerHTML = new Date(Date.now()).toLocaleString();
